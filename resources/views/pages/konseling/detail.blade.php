...
@section('body')
<header>
    <x-main-header title="Konseling Individu Detail" />
    <x-breadcrumb :datas="[route('konseling.index') => 'Konseling']" last="Detail" />
</header>
        ...
        <div class="mb-5">
            ...
            <select name="siswaId" id="siswa" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                ...
                @foreach ($siswas as $item)
                    <option value="{{ $item->id }}" @selected($item->id == $konseling->siswa_id)>{{ $item->NIS }} | {{ $item->nama }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-5">
...
<section class="w-full p-4 border border-gray-100 shadow rounded-lg mt-10">
    ...
    <div class="flex justify-center items-center">
        <a href="{{ route('konseling.delete', $konseling->id) }}" class="block w-min text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button">
            Hapus
        </a>
    </div>
</section>
