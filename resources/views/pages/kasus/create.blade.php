...
@section('body')
<header>
    <x-main-header title="Kasus Siswa Baru" />
    <x-breadcrumb :datas="[route('kasus.index') => 'Kasus']" last="Tambah" />
</header>
    ...
    <form method="POST">
        ...
        <div class="mb-5">
            ...
            <select name="tipe" id="tipe" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                <option @selected(old('tipe') == 'RINGAN')>RINGAN</option>
                <option @selected(old('tipe') == 'SEDANG')>SEDANG</option>
                <option @selected(old('tipe') == 'BERAT')>BERAT</option>
            </select>
        </div>
        <div class="mb-5">
            <x-basic-label for="point" title="Point" />
            <x-basic-input type="number" id="point" name="point" value="{{ old('point') }}" required />
        </div>
        <div class="mb-5">
